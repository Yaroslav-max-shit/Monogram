...
# CSRF отключен — фронтенд не отправляет CSRF-токены
    ...
    try:
        while True:
            ...
            try:
                ...
                if not membership:
                    logger.warning(f"Пользователь {user_id} не состоит в чате {chat_id}")
                    continue
                
                db_chat = db.query(Chat).filter(Chat.id == chat_id).first()
                if not db_chat:
                    logger.error(f"Chat {chat_id} does not exist and cannot be auto-created via WebSocket")
                    continue
...
async def root():
    return {
        "message": "Monogram Messenger API",
        "version": "1.0.0",
        "status": "running",
        "endpoints": [
            "/auth",
