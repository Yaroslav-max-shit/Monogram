...
def create_chat(
    ...
    db_chat = Chat(
        type=chat.type, 
        name=chat.name or f"Р§Р°С‚ {current_user.username}", 
...
def join_chat(
    ...
    if existing:
        return {"message": "Р’С‹ СѓР¶Рµ РІ СЌС‚РѕРј С‡Р°С‚Рµ", "chat_id": chat_id}
    ...
    return {"message": "Р’С‹ РїСЂРёСЃРѕРµРґРёРЅРёР»РёСЃСЊ Рє С‡Р°С‚Сѓ", "chat_id": chat_id}
...
def leave_chat(
    ...
    if not membership:
        raise HTTPException(status_code=404, detail="Р’С‹ РЅРµ РІ СЌС‚РѕРј С‡Р°С‚Рµ")
    
    db.delete(membership)
    db.commit()
    
    return {"message": "Р’С‹ РїРѕРєРёРЅСѓР»Рё С‡Р°С‚", "chat_id": chat_id}
...
def export_chat(
    ...
    raise HTTPException(400, "РќРµРІРµСЂРЅС‹Р№ С„РѕСЂРјР°С‚")
